...
class ChatPanel {
    ...
    _disposables = [];
    _history = [];
    constructor(panel, api) {
        this._panel = panel;
        this._api = api;
        this._agent = new agentRunner_1.AgentRunner(api);
        this._panel.onDidDispose(() => this.dispose(), null, this._disposables);
        this._panel.webview.onDidReceiveMessage(this._handleMessage.bind(this), null, this._disposables);
        this._panel.webview.html = this._getHtml();
    }
    static createOrShow(api) {
        const column = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.viewColumn : undefined;
        if (ChatPanel.currentPanel) {
            ChatPanel.currentPanel._panel.reveal(column);
            return;
        }
        const panel = vscode.window.createWebviewPanel('lmstudioChat', 'ModelLane', column || vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true, localResourceRoots: [] });
        ...
        switch (msg.type) {
            case 'sendMessage':
                if (typeof msg.text === 'string')
                    await this._handleSend(msg.text, msg.agentMode === true);
                break;
            case 'cancelRequest':
    ...
    async _insertCode(code) {
        ...
        if (!editor)
            return;
        const picked = await vscode.window.showWarningMessage('Insert generated code into the active editor?', { modal: true }, 'Insert');
        if (picked === 'Insert') {
