...
class LMStudioApi {
    ...
    remoteBaseUrlIsUserConfigured = false;
    ...
    async autoSelectModel(signal) {
        const models = await this.listModels(signal);
        if (models.length === 0)
            throw new Error('No models found in LM Studio. Make sure a model is loaded.');
        const loadedModels = models.filter(m => m.loaded);
        const candidates = loadedModels.length > 0 ? loadedModels : models;
        const preferred = candidates.find(m => m.id.toLowerCase().includes('qwen') ||
    ...
    async *chatStream(messages, modelOverride) {
        ...
        try {
            ...
            if (!model)
                model = await this.autoSelectModel(signal);
            ...
            if (!res.body)
                throw new Error('LM Studio API returned an empty response body.');
    ...
    buildChatBody(model, messages, maxTokens, temperature) {
        if (this.apiMode === 'native') {
            return {
                model,
    ...
    messagesToNativeInput(messages) {
    ...
    buildEndpoint(path) {
        ...
        if (!this.isLoopbackHost(url.hostname) && (!this.allowRemoteBaseUrl || !this.remoteBaseUrlIsUserConfigured)) {
            throw new Error(`Refusing to send code to non-local LM Studio API host "${url.hostname}". ` +
...
//# sourceMappingURL=lmStudioApi.js.map